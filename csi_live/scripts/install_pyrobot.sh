...
if [ $PYTHON_VERSION == "3" ]; then
	...
	if [ ! -d "$VIRTUALENV_FOLDER" ]; then
		...
		sudo apt-get update
		sudo apt-get -y install python-catkin-tools python3.7-dev python3-catkin-pkg-modules python3-numpy python3-yaml
		sudo apt-get -y install python3-tk python3.7-tk
		virtualenv -p /usr/bin/python3.7 $VIRTUALENV_FOLDER
		source ~/${virtualenv_name}/bin/activate
	...
	if [ ! -d "$PYROBOT_PYTHON3_WS/src" ]; then
		...
		# Build
		catkin_make --cmake-args -DPYTHON_EXECUTABLE=$(which python) -DPYTHON_INCLUDE_DIR=/usr/include/python3.7m -DPYTHON_LIBRARY=/usr/lib/x86_64-linux-gnu/libpython3.7m.so
