...
def pcap_to_csi(pcap, chip, clients, cores, streams):
    """Converts pcap data to Numpy array of CSI.

    Args:
        pcap: .pcap file to convert to CSI.
        chip: Monitor chip.
        client: List of client MAC addresses.
        cores: List of cores used. 
        streams: List of spatial streams used. 

    Returns:
        A list of n_core x n_stream x n_samples x n_subc complex64 Numpy arrays
        of CSI values corresponding to each client.

    """
    pcap = io.BytesIO(pcap)
    frames = dpkt.pcap.Reader(pcap)    

    # Initialize empty lists to hold result. 
    csi = [[[[] for _ in streams] for _ in cores] for _ in clients]

    # Fill lists.
    for _, frame in frames:
        payload = dpkt.ethernet.Ethernet(frame).data.data.data
        unpacked_csi = unpack_csi(payload,chip)

        client_idx = clients.index(unpacked_csi["src_mac"])
        core_idx = cores.index(unpacked_csi["core"])
        stream_idx = streams.index(unpacked_csi["stream"])

        csi[client_idx][core_idx][stream_idx].append(unpacked_csi['data'])

    csi = [np.array(client_csi) for client_csi in csi]
